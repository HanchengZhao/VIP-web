...
const PageLength = 3; // 4 announcements per page
...
class AnnouncementList extends Component {
  ...
  componentDidMount(){
    axios.get(AnnouncementRef) // only fetch uuids
    .then((res) => {
        ...
        <div className="row">
          <Link to={"announcement/creation"}>
            <MuiThemeProvider muiTheme={getMuiTheme(darkBaseTheme)}>
              <RaisedButton label = "Create an announcement" id = "applyButton" backgroundColor = "#ffc425" style = {{float: "right", margin:"10px"}}/>
            </MuiThemeProvider>
          </Link>
        </div>
        }
